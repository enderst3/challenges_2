...
def find_even_index(arr):
    # for loop to go through the list and compare the sums of each side of the list
    ...
    # List comp solution
    result = [i for i in range(len(arr)) if sum(arr[0:i]) == sum(arr[i+1:])]
    # if there is a result return result, if not return -1
    return result[i] if result else -1
